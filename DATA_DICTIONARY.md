...
| 29 | **`Parcels in sale`** | `Int64` | *Derived* | No (`null`) | How many rows share this `Dealing number` — i.e. how many parcels the one sale covered. `1` for a normal sale (4,854,282 rows), up to 195 for the largest. Null pre-2001, where it is unknowable. See Caveat 5. |
...
115,054 dealings cover more than one parcel; each parcel is its own row carrying the **full** sale price. That repeats the price on **287,179 rows**, and the largest single sale spans 195 parcels. Summing or taking a median across them double-counts. Filter with `df['Parcels in sale'] == 1`, or group by `Dealing number` and take one row per sale.

> Earlier versions of this document put these at 50,357 dealings across 72,631 rows. Those were measured before the deduplication key included `Property legal description`, so most parcel rows had already been deleted by the time anyone counted them — the old figures understated the problem by roughly a factor of four.
...
6. **Cross-Batch Deduplication**:
   ...
   - Deduplication then splits on **whether the record carries a `Dealing number`** - never on its date, because contract dates are unreliable (Caveat 3):
     - **With a dealing number** (post-2001): keyed on `['Property ID', 'Dealing number', 'Property legal description']`. The legal description is essential: one dealing routinely covers several parcels and each parcel is its own row, so a key of property-plus-dealing alone destroys every parcel but one (99,427 records across the corpus). With the description included, the key still collapses the restatements - the Valuer General republishing the same parcel later - while keeping parcels that are genuinely different.
